...
class DataraperkadaController extends Controller
{
    ...
    public function storeraperkada($kode_pereseanuser, Request $request)
    {
        $getdata_user = DB::table('tbl_user')->where('kode_pereseanuser', $kode_pereseanuser)->first();
        $id_user = $getdata_user->id_user;
        $username = $getdata_user->username;

        $get_idzona_user = $getdata_user->id_zona;
        $get_namazona_user = DB::table('tbl_zona')->where('id_zona', $get_idzona_user)->first();

        $nama_draft_raperkada = $request->nama_draft_raperkada;
        $fileName = "";
        $count_if_exist_daduk = 0;
        $cek_namadraft = DB::table('tbl_draft')
            ->where('nama_draft_permohonan', $nama_draft_raperkada)
            ->where('kode_pereseanuser', $kode_pereseanuser);


        if ($cek_namadraft->count() > 0) {
            return redirect()->route('getraperkada', $kode_pereseanuser)->with(['warning' => 'Data Gagal Disimpan,Nama Draft Sudah Ada']);
        } else if ($cek_namadraft->count() <= 0) {
            /*UNTUK GET NAMA FILE SAJA*/
            if ($request->hasFile('surat_permohonan_raperkada')) {
                $sign_code_spa = "SPA";
                $tgl_lengkap = date('Y-m-d H:i:s');
                $unique_code = explode(" ", $tgl_lengkap);
                $unique_code_name = str_replace(':', '_', $unique_code[1]);
                $fileName = pathinfo($request->surat_permohonan_raperkada->getClientOriginalName(), PATHINFO_FILENAME) .
                    '-' . auth()->user()->id_user . "_" . $unique_code_name . '_' . $sign_code_spa .
                    '.' . $request->surat_permohonan_raperkada->getClientOriginalExtension();
                $surat_permohonan_raperkada_temp = $fileName;
                $pathSuratPermohonan = "public/uploads/permohonan_raperkada/$username/$surat_permohonan_raperkada_temp";
                //echo $pathSuratPermohonan."<br>";
                //echo "tes has file surat permohonan raperkada";
                if (Storage::exists($pathSuratPermohonan)) {
                    //echo "File exists";
                } else {
                    //echo "File doesnt exists";
                }
            } else {
                $surat_permohonan_raperkada_temp = "no data";
            }
            //die;

            if ($request->hasFile('surat_naskahakademik_etc')) {
                $sign_code_sne = "SNE";
                $tgl_lengkap_naskahakademik = date('Y-m-d H:i:s');
                $unique_code_naskahakademik = explode(" ", $tgl_lengkap_naskahakademik);
                $unique_code_name_naskahakademik = str_replace(':', '_', $unique_code_naskahakademik[1]);
                $fileName_naskahakademik = pathinfo($request->surat_naskahakademik_etc->getClientOriginalName(), PATHINFO_FILENAME) .
                    '-' . auth()->user()->id_user . "_" . $unique_code_name_naskahakademik . '_' . $sign_code_sne .
                    '.' . $request->surat_naskahakademik_etc->getClientOriginalExtension();
                $surat_naskahakademik_temp = $fileName_naskahakademik;
            } else {
                $surat_naskahakademik_temp = "no data";
            }
            $surat_naskahakademik_tosave = json_encode($surat_naskahakademik_temp);

            if ($request->hasFile('rancangan_perkada')) {
                $sign_code_rpk = "RPK";
                $tgl_lengkap_rancangan_perkada = date('Y-m-d H:i:s');
                $unique_code_rancangan_perkada = explode(" ", $tgl_lengkap_rancangan_perkada);
                $unique_code_name_rancangan_perkada = str_replace(':', '_', $unique_code_rancangan_perkada[1]);
                $fileName_rancangan_perkada = pathinfo($request->rancangan_perkada->getClientOriginalName(), PATHINFO_FILENAME) .
                    '-' . auth()->user()->id_user . "_" . $unique_code_name_rancangan_perkada . '_' . $sign_code_rpk .
                    '.' . $request->rancangan_perkada->getClientOriginalExtension();
                $rancangan_perkada_temp = $fileName_rancangan_perkada;
            } else {
                $rancangan_perkada_temp = "no data";
            }
            $surat_rancangan_perkada_tosave = json_encode($rancangan_perkada_temp);

            if ($request->hasFile('draft_raperkada')) {
                $sign_code_drp = "DRP";
                $tgl_lengkap_draft_raperkada = date("Y-m-d H:i:s");
                $unique_code_draft_raperkada = explode(" ", $tgl_lengkap_draft_raperkada);
                $unique_code_name_draft_raperkada = str_replace(':', '_', $unique_code_draft_raperkada[1]);
                $fileName_draft_raperkada = pathinfo($request->draft_raperkada->getClientOriginalName(), PATHINFO_FILENAME) .
                    '-' . auth()->user()->id_user . "_" . $unique_code_name_draft_raperkada . '_' . $sign_code_drp .
                    '.' . $request->draft_raperkada->getClientOriginalExtension();
                $draft_raperkada_temp = $fileName_draft_raperkada;
            } else {
                $draft_raperkada_temp = "no data";
            }
            $draft_raperkada_tosave = json_encode($draft_raperkada_temp);

            $count_if_exist_lampiran_tambahan = 0;
            $counting_lampiran_tambahan = count((array)$request->lampiran_tambahan);

            //echo "jumlah lampiran tambahan peresean_new: ".$counting_lampiran_tambahan; die;
            /*NB : $counting_lampiran_tambahan hanya akan terbaca jika field input sudah melakukan select terhadap file yg akan di upload*/
            //echo count((array)($request->lampiran_tambahan))." woy"; die;
            //$answers = array_filter(Input::get('answer'));
            //echo $counting_lampiran_tambahan; die;

            if (count((array)$request->jumlah_daduk) <= 0) {

                $count_daduk = 0;
            } else if (count($request->jumlah_daduk) > 0) {
                for ($a = 0; $a < count($request->jumlah_daduk); $a++) {
                    $count_if_exist_daduk += 1;
                }
                $count_daduk = $count_if_exist_daduk;
            }

            //echo "request daduks: ".$count_daduk; die;
            $array_gabung_lampiran_daduk = [];
            if ($count_daduk != 0) {
                if ($request->hasFile('jumlah_daduk')) {
                    for ($j = 0; $j < $count_daduk; $j++) {
                        if (isset($request->jumlah_daduk[$j])) {
                            $sign_code_lamptamb = "LMT_" . $j;
                            $tgl_lengkap_lamp_tambahan = date('Y-m-d H:i:s');
                            $unique_code_lamp_tambahan = explode(' ', $tgl_lengkap_lamp_tambahan);
                            $unique_code_name_lamp_tambahan = str_replace(':', '-', $unique_code_lamp_tambahan[1]);
                            $fileName_lamp_tambahan = pathinfo($request->jumlah_daduk[$j]->getClientOriginalName(), PATHINFO_FILENAME) .
                                '-' . auth()->user()->id_user . "_" . $unique_code_name_lamp_tambahan . '_' . $sign_code_lamptamb .
                                '.' . $request->jumlah_daduk[$j]->getClientOriginalExtension();
                            $folderPath_lamp_tambahan = "public/uploads/permohonan_raperkada/$username";
                            //$request->lampiran_tambahan[$j]->storeAs($folderPath_lamp_tambahan, $fileName_lamp_tambahan);
                            $array_gabung_lampiran_daduk[$j] = $fileName_lamp_tambahan;
                        } else if (!isset($request->jumlah_daduk[$j])) {
                            $array_gabung_lampiran_daduk[$j] = "no data";
                        }
                    }
                } else {
                    for ($j = 0; $j < $count_daduk; $j++) {
            ...
            try {
                $data = [
                    ...
                    "nama_draft_permohonan" => $nama_draft_raperkada,
                    'status' => 'belum_diproses',
                    'jenis_dokumen' => $request->jenis_dokumen,
                    'kode_pereseanuser' => $kode_pereseanuser,
                    'lamp_surat_permohonan' => $surat_permohonan_raperkada_temp,
                    'naskah_akademik_dll' => $surat_naskahakademik_temp,
                    'rancangan_perkada' => $rancangan_perkada_temp,
                    'draft_harmonisasi' => $draft_raperkada_temp,
                    'url_data_dukung' => json_encode($array_gabung_lampiran_daduk),

                    "id_user" => $id_user,
                    "zona_dokumen" => $get_namazona_user->nama_zona,
                ...
                $simpan = DB::table('tbl_draft')->insert($data);
                if ($simpan) {
                    /*UNTUK STORE FILE SAJA*/
                    $folderPath = "public/uploads/permohonan_raperkada/$username";
                    if ($request->hasFile('surat_permohonan_raperkada')) {
                        $request->surat_permohonan_raperkada->storeAs($folderPath, $fileName);
                    }
                    if ($request->hasFile('surat_naskahakademik_etc')) {
                        $request->surat_naskahakademik_etc->storeAs($folderPath, $fileName_naskahakademik);
                    }

                    if ($request->hasFile('rancangan_perkada')) {
                        $request->rancangan_perkada->storeAs($folderPath, $fileName_rancangan_perkada);
                    }
                    if ($request->hasFile('draft_raperkada')) {
                        $request->draft_raperkada->storeAs($folderPath, $fileName_draft_raperkada);
                    }
                    if ($request->hasFile('jumlah_daduk')) {
                        for ($k = 0; $k < $count_daduk; $k++) {
                            $sign_code_lamptamb_filename = "LMT_" . $k;
                            $tgl_lengkap_lamptambahan = date('Y-m-d H:i:s');
                            $unique_code_lamptambahan = explode(' ', $tgl_lengkap_lamptambahan);
                            $unique_code_name_lamptambahan = str_replace(':', '-', $unique_code_lamptambahan[1]);
                            $fileName_lamptambahan = pathinfo($request->jumlah_daduk[$k]->getClientOriginalName(), PATHINFO_FILENAME) .
                                '-' . auth()->user()->id_user . "_" . $unique_code_name_lamptambahan . '_' . $sign_code_lamptamb_filename .
                                '.' . $request->jumlah_daduk[$k]->getClientOriginalExtension();
                            $folderPathLampTambahan = "public/uploads/permohonan_raperkada/$username";
                            $request->jumlah_daduk[$k]->storeAs($folderPathLampTambahan, $fileName_lamptambahan);
                        }
                    }
                    return redirect()->route('getraperkada', $kode_pereseanuser)->with(['success' => 'Data Draft Raperkada Berhasil Disimpan']);
                } else {
            ...
            //echo "<pre>";
            //print_r($array_gabung_lampiran_daduk);
            //die;
            /*contoh dari sini*/
    ...
    public function replacedata($kode_pereseanuser, Request $request)
    {
        return $request;
    }

    public function updateraperkada($kode_pereseanuser, Request $request)
    {
        $id_draft_permohonan = decrypt($request->id_draft_permohonan);
        $nama_draft_raperkada = $request->nama_draft_raperkada;
        $jenis_dokumen = $request->jenis_dokumen;
        $url_file = [];
        $simpan = "n";
        ...
        $getdata_user = DB::table('tbl_user')->where('kode_pereseanuser', $kode_pereseanuser)->first();
        //echo "<pre>"; print_r($getdata_user); die;
        $id_user = $getdata_user->id_user;
        $username = $getdata_user->username;
        //echo $username; die;

        $get_idzona_user = $getdata_user->id_zona;
        //echo $get_idzona_user; die;
        $get_namazona_user = DB::table('tbl_zona')->where('id_zona', $get_idzona_user)->first();
        $nama_zona = $get_namazona_user->nama_zona;
        //echo $nama_zona; die;

        $get_olddata = DB::table('tbl_draft')->where('id_draft_permohonan', $id_draft_permohonan)->first();
        /*tricky, filter dulu mana yg bukan nama draft permohonan lama, lalu filter lg mana yg sama,klo lebih besar dari 1 maka gagal*/
        $get_raperkadaname = DB::table('tbl_draft')
            ->where('nama_draft_permohonan', '!=', $get_olddata->nama_draft_permohonan)
            ->where('nama_draft_permohonan', $nama_draft_raperkada);
        //echo $get_raperkadaname->count(); die;
        if ($get_raperkadaname->count() > 0) {
            $simpan = "n";
        } else if ($get_raperkadaname->count() <= 0) {
            $simpan = "y";
        }
        //echo $get_olddata->nama_draft_permohonan;
        //die;

        $old_lamp_surat_permohonan = $get_olddata->lamp_surat_permohonan;
        $old_naskah_akademik_dll = $get_olddata->naskah_akademik_dll;
        $old_rancangan_perkada = $get_olddata->rancangan_perkada;
        $old_draft_harmonisasi = $get_olddata->draft_harmonisasi;
        $old_url_data_dukung = json_decode($get_olddata->url_data_dukung);

        /*LAMPIRAN SURAT PERMOHONANAN*/
        if ($request->hasFile('surat_permohonan_raperkada')) {
            $sign_code_spa = "SPA";
            $tgl_lengkap = date('Y-m-d H:i:s');
            $unique_code = explode(" ", $tgl_lengkap);
            ...
            $fileName = pathinfo($request->surat_permohonan_raperkada->getClientOriginalName(), PATHINFO_FILENAME) .
                ...
                '.' . $request->surat_permohonan_raperkada->getClientOriginalExtension();
            $surat_permohonan_raperkada_temp = $fileName;
            //filename itu file yg baru
        } else {
            $surat_permohonan_raperkada_temp = $old_lamp_surat_permohonan;
            //$old_lamp_surat_permohonan itu file yg lama
        }
        //echo $surat_permohonan_raperkada_temp; die;

        /*NASKAH AKADEMIK DLL*/
        if ($request->hasFile('surat_naskahakademik_etc')) {
            $sign_code_sne = "SNE";
            $tgl_lengkap_naskah = date('Y-m-d H:i:s');
            $unique_code_naskah = explode(" ", $tgl_lengkap_naskah);
            $unique_code_name_naskah = str_replace(':', '_', $unique_code_naskah[1]);
            $fileName_naskah = pathinfo($request->surat_naskahakademik_etc->getClientOriginalName(), PATHINFO_FILENAME) .
                '-' . auth()->user()->id_user . "_" . $unique_code_name_naskah . '_' . $sign_code_sne .
                '.' . $request->surat_naskahakademik_etc->getClientOriginalExtension();
            $naskah_temp = $fileName_naskah;
        } else {
            $naskah_temp = $old_naskah_akademik_dll;
        }

        /*RANCANGAN PERKADA*/
        if ($request->hasFile('rancangan_perkada')) {
            $sign_code_rpk = "RPK";
            $tgl_lengkap_rancangan = date('Y-m-d H:i:s');
            $unique_code_rancangan = explode(" ", $tgl_lengkap_rancangan);
            $unique_code_name_rancangan = str_replace(':', '_', $unique_code_rancangan[1]);
            $fileName_rancangan = pathinfo($request->rancangan_perkada->getClientOriginalName(), PATHINFO_FILENAME) .
                '-' . auth()->user()->id_user . "_" . $unique_code_name_rancangan . '_' . $sign_code_rpk .
                '.' . $request->rancangan_perkada->getClientOriginalExtension();
            $rancangan_temp = $fileName_rancangan;
        } else {
            $rancangan_temp = $old_rancangan_perkada;
        }

        /*DRAFT HARMONISASI*/
        if ($request->hasFile('draft_raperkada')) {
            $sign_code_drp = "DRP";
            $tgl_lengkap_draft = date('Y-m-d H:i:s');
            $unique_code_draft = explode(" ", $tgl_lengkap_draft);
            $unique_code_name_draft = str_replace(':', '_', $unique_code_draft[1]);
            $fileName_draft = pathinfo($request->draft_raperkada->getClientOriginalName(), PATHINFO_FILENAME) .
                '-' . auth()->user()->id_user . "_" . $unique_code_name_draft . '_' . $sign_code_drp .
                '.' . $request->draft_raperkada->getClientOriginalExtension();
            $draft_temp = $fileName_draft;
        } else {
            $draft_temp = $old_draft_harmonisasi;
        }


        $counting_lampiran_tambahan = count((array)$request->tambahan_files);

        //echo $counting_lampiran_tambahan; die;
        if ($counting_lampiran_tambahan != 0) {
            for ($i = 0; $i < $counting_lampiran_tambahan; $i++) {
                if ($request->hasFile('tambahan_files')) {
                    //echo "terset brur! tambahannya.<br>";
                    /*FILENAME MAKER ONLY*/
                    if (isset($request->tambahan_files[$i])) {
                        $sign_code_lamptamb = "LMT_REV_" . $i;
                        $tgl_lengkap_lamp_tambahan = date('Y-m-d H:i:s');
                        $unique_code_lamp_tambahan = explode(' ', $tgl_lengkap_lamp_tambahan);
                        $unique_code_name_lamp_tambahan = str_replace(':', '-', $unique_code_lamp_tambahan[1]);
                        $fileName_lamp_tambahan = pathinfo($request->tambahan_files[$i]->getClientOriginalName(), PATHINFO_FILENAME) .
                            '-' . auth()->user()->id_user . "_" . $unique_code_name_lamp_tambahan . '_' . $sign_code_lamptamb .
                            '.' . $request->tambahan_files[$i]->getClientOriginalExtension();
                        $folderPath_lamp_tambahan = "public/uploads/permohonan_raperkada/$username";
                        //$request->lampiran_tambahan[$j]->storeAs($folderPath_lamp_tambahan, $fileName_lamp_tambahan);
                        $url_file[$i] = $fileName_lamp_tambahan;
                    }
                }
            }
            $file_lama = json_decode($get_olddata->url_data_dukung == 'null' ? "[]" : $get_olddata->url_data_dukung);
            $url_data_dukung = json_encode(array_merge($file_lama, $url_file));
        } else {
            $url_data_dukung = $get_olddata->url_data_dukung;
        }

        //echo "<pre>"; print_r($url_data_dukung);
        //die;
        ...
        if ($simpan == "y") {
            try {
                $data_to_update = [
                    ...
                    'kode_pereseanuser' => $kode_pereseanuser,
                    'lamp_surat_permohonan' => $surat_permohonan_raperkada_temp,
                    'naskah_akademik_dll' => $naskah_temp,
                    'rancangan_perkada' => $rancangan_temp,
                    'draft_harmonisasi' => $draft_temp,
                ...
                //echo $old_lamp_surat_permohonan . "=>" . $fileName . "<br>";
                //echo $old_naskah_akademik_dll . "=>" . $fileName_naskah . "<br>";
                //echo $old_rancangan_perkada . "=>" . $fileName_rancangan . "<br>";
                //echo $old_draft_harmonisasi . "=>" . $fileName_draft . "<br>";

                //echo "<pre>"; print_r($url_data_dukung);
//            foreach (json_decode($url_data_dukung) as $idx=>$itm){
//                echo $itm."<br>";
//            }


                $update_tbl_draft = DB::table('tbl_draft')->where('id_draft_permohonan', $id_draft_permohonan)->update($data_to_update);
                if ($update_tbl_draft) {
                    /*STORE FILE ONLY*/
                    $folderPath = "public/uploads/permohonan_raperkada/$username";
                    ...
                    if ($request->hasFile('surat_permohonan_raperkada')) {
                        ...
                        /*DELETE OLD FILE*/
                        $folderLampiranPermohonan_todelete = "public/uploads/permohonan_raperkada/$username/" . $old_lamp_surat_permohonan;
                        Storage::delete($folderLampiranPermohonan_todelete);
                        //echo $fileName;
                    }

                    if ($request->hasFile('surat_naskahakademik_etc')) {
                        /*SAVE NEW FILE*/
                        $request->surat_naskahakademik_etc->storeAs($folderPath, $fileName_naskah);

                        /*DELETE OLD FILE*/
                        $folderLampiranNaskah_todelete = "public/uploads/permohonan_raperkada/$username/" . $old_naskah_akademik_dll;
                        Storage::delete($folderLampiranNaskah_todelete);
                    }

                    if ($request->hasFile('rancangan_perkada')) {
                        /*SAVE NEW FILE*/
                        $request->rancangan_perkada->storeAs($folderPath, $fileName_rancangan);

                        /*DELETE OLD FILE*/
                        $folderLampiranRaperkada_todelete = "public/uploads/permohonan_raperkada/$username/" . $old_rancangan_perkada;
                        Storage::delete($folderLampiranRaperkada_todelete);
                    }

                    if ($request->hasFile('draft_raperkada')) {
                        /*SAVE NEW FILE*/
                        $request->draft_raperkada->storeAs($folderPath, $fileName_draft);

                        /*DELETE OLD FILE*/
                        $folderLampiranDraft_todelete = "public/uploads/permohonan_raperkada/$username/" . $old_draft_harmonisasi;
                        Storage::delete($folderLampiranDraft_todelete);
                    }

                    if ($counting_lampiran_tambahan != 0) {
                        for ($m = 0; $m < $counting_lampiran_tambahan; $m++) {
                            if ($request->hasFile('tambahan_files')) {
                                if (isset($request->tambahan_files[$m])) {
                                    $sign_code_lamptamb = "LMT_REV_" . $m;
                                    echo $sign_code_lamptamb . "<br>";
                                    $tgl_lengkap_lamp_tambahan = date('Y-m-d H:i:s');
                                    $unique_code_lamp_tambahan = explode(' ', $tgl_lengkap_lamp_tambahan);
                                    $unique_code_name_lamp_tambahan = str_replace(':', '-', $unique_code_lamp_tambahan[1]);
                                    $fileName_lamp_tambahan = pathinfo($request->tambahan_files[$m]->getClientOriginalName(), PATHINFO_FILENAME) .
                                        '-' . auth()->user()->id_user . "_" . $unique_code_name_lamp_tambahan . '_' . $sign_code_lamptamb .
                                        '.' . $request->tambahan_files[$m]->getClientOriginalExtension();
                                    $folderPath_lamp_tambahan = "public/uploads/permohonan_raperkada/$username";
                                    $request->tambahan_files[$m]->storeAs($folderPath_lamp_tambahan, $fileName_lamp_tambahan);
                                    $url_file[$m] = $fileName_lamp_tambahan;
                                }
                            }

                        }

                        // echo $id_draft_permohonan;
                        //echo "<pre>"; print_r($get_olddata->url_data_dukung);
                        //return redirect()->route('getraperkada', $kode_pereseanuser)->with(['success' => 'Data Draft Raperkada Berhasil Diupdate']);
                    } else if ($counting_lampiran_tambahan <= 0) {
                        //return redirect()->route('getraperkada', $kode_pereseanuser)->with(['success' => 'Data Draft Raperkada Berhasil Diupdate']);
                        //echo "counting lampiran tambahan : " . $counting_lampiran_tambahan;
                    }
                    return redirect()->route('getraperkada', $kode_pereseanuser)->with(['success' => 'Data Draft Raperkada Berhasil Diupdate']);
                } else {
                    return redirect()->route('getraperkada', $kode_pereseanuser)->with(['warning' => 'Data Draft Raperkada Gagal Diupdate']);
                }
    ...
    public function getraperkada($kode_pereseanuser, Request $request)
    {
        ...
        if (auth()->user()->level == 'superadmin') {
            ...
            return view('raperkadauser.index_raperkada', compact('draft_raperkada', 'pereseanuser'
                , 'kode_pereseanuser', 'fullname_bykodepereseanuser'));
        } else {
    ...
    public function editraperkada($kode_pereseanuser, $id_draft_permohonan, Request $request)
    {
        $get_username_pemda_terkait = DB::table('tbl_user')->where('kode_pereseanuser', $kode_pereseanuser)->first();
        $username_pemda_terkait = $get_username_pemda_terkait->username;
        $data_raperkada = DB::table('tbl_draft')->where('id_draft_permohonan', decrypt($id_draft_permohonan))->first();
        $jml_data_exist = count(json_decode($data_raperkada->url_data_dukung));
        //echo count(json_decode($data_raperkada->url_data_dukung));
        //echo "<pre>"; print_r($data_raperkada->url_data_dukung);die;
        //echo count(json_decode($data_raperkada->url_data_dukung)); die;
        //echo "<pre>"; print_r($data_raperkada); die;
        //echo "<pre>"; print_r($data_raperkada); die;
        return view('raperkadauser.edit_raperkada', compact('kode_pereseanuser', 'data_raperkada'
            , 'username_pemda_terkait', 'id_draft_permohonan', 'jml_data_exist'));
    }

    public function hapusdatalampiran($id_draft_permohonan, Request $request)
    {
        $dokumen_tambahan = $request->dokumen_tambahan;
        $key_tambahan = $request->key_tambahan;
        $id_draft_permohonan = $request->id_draft_permohonan;
        $kode_pereseanuser = $request->kode_pereseanuser;
        $peresean_username = DB::table("tbl_user")->where("kode_pereseanuser", $kode_pereseanuser)->first()->username;
        //return $peresean_username;
        //return $request;
        $get_data_permohonanraperkada = DB::table("tbl_draft")->where('id_draft_permohonan', $id_draft_permohonan)->first();
        //echo json_decode($get_data_permohonanraperkada->url_data_dukung); die;
        //echo "<pre>"; print_r($get_data_permohonanraperkada); die;
        $get_lampiran_tambahan = json_decode($get_data_permohonanraperkada->url_data_dukung);
        //return $get_lampiran_tambahan;
        if ($get_lampiran_tambahan[$key_tambahan] != "") {
            $lampiran_tambahan_todelete = $get_lampiran_tambahan[$key_tambahan];
            $folderLampiranTambahanOld = "public/uploads/permohonan_raperkada/$peresean_username/" . $lampiran_tambahan_todelete;

            //perintah delete file pada storage
            Storage::delete($folderLampiranTambahanOld);

            if (($key = array_search($get_lampiran_tambahan[$key_tambahan], $get_lampiran_tambahan)) !== false) {
                unset($get_lampiran_tambahan[$key_tambahan]);
            }
        } else if ($get_lampiran_tambahan[$key_tambahan] == "") {
            if (($key = array_search($get_lampiran_tambahan[$key_tambahan], $get_lampiran_tambahan)) !== false) {
                unset($get_lampiran_tambahan[$key_tambahan]);
            }
        }
        ...
        try {
            $data_to_update = [
                "url_data_dukung" => json_encode(count($get_lampiran_tambahan) > 0 ? array_values($get_lampiran_tambahan) : []),
                "tgl_update" => date("Y-m-d H:i:s"),
            ];

            $update = DB::table('tbl_draft')->where('id_draft_permohonan', $id_draft_permohonan)->update($data_to_update);
            if ($update) {
                //return redirect()->route('getraperkada', $kode_pereseanuser)->with(['success' => 'Data Draft Raperkada Berhasil Disimpan']);
                //Alert::success('Success', 'Data Berhasil Dihapus');
                return redirect()->route('editraperkada', [$kode_pereseanuser, encrypt($id_draft_permohonan)])->with(['success' => 'Data Lampiran Berhasil Dihapus']);
            } else {
                //Alert::error('Gagal Menghapus Data', 'Gagal');
                return redirect()->route('editraperkada', [$kode_pereseanuser, encrypt($id_draft_permohonan)])->with(['warning' => 'Data Lampiran Gagal Dihapus']);
            }
        } catch (Exception $e) {
            return redirect()->route('editraperkada', [$kode_pereseanuser, encrypt($id_draft_permohonan)])->with(['warning' => 'Data Lampiran Tambahan Gagal Dihapus ,error:' . $e]);
        }
    }

    public function addraperkada($kode_pereseanuser, Request $request)
    {
        $pereseanuser = DB::table('tbl_user')->where('kode_pereseanuser', $kode_pereseanuser)->first();
        $namalengkap_bykodepereseanuser = $pereseanuser->nama_lengkap;
        return view('raperkadauser.add_raperkada', compact('kode_pereseanuser', 'namalengkap_bykodepereseanuser'));
    }
